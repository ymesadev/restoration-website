...
interface LeadFormProps {
  ...
  className?: string;
  /** Tailwind color name for styling (teal, blue, orange) */
  accentColor?: string;
}
...
export default function LeadForm({ variant, services = [], className = '', accentColor = 'blue' }: LeadFormProps) {
    ...
    return (
      <form onSubmit={handleSubmit} className={`space-y-4 ${className}`}>
        <input name="name" type="text" placeholder="Your Name" required value={form.name} onChange={handleChange}
          className={`w-full px-4 py-3 rounded-lg bg-white/10 border border-white/20 text-white placeholder-${accentColor}-200 focus:outline-none focus:border-white/50`} />
        ...
        <select name="service" required value={form.service} onChange={handleChange}
          className={`w-full px-4 py-3 rounded-lg bg-white/10 border border-white/20 text-${accentColor}-200 focus:outline-none focus:border-white/50`}>
        ...
        <div className="flex items-start gap-3">
          <input type="checkbox" id="hero-optin" name="optin" checked={form.optin} onChange={handleChange}
            className="mt-1 h-4 w-4 text-amber-500 border-white/30 rounded focus:ring-amber-500" />
          <label htmlFor="hero-optin" className={`text-sm text-${accentColor}-200`}>
            I agree to receive text messages and calls regarding my inquiry. Message and data rates may apply.
          </label>
        </div>
        <button type="submit" disabled={status === 'submitting'}
          className="w-full bg-amber-500 hover:bg-amber-600 text-white font-semibold py-4 rounded-lg transition disabled:opacity-50">
          {status === 'submitting' ? 'Submitting...' : 'Get My Free Quote'}
        </button>
        {status === 'error' && <p className="text-red-300 text-sm text-center">Something went wrong. Please try again or call us.</p>}
      </form>
    );
  }
  ...
  if (variant === 'landing') {
    return (
      <form onSubmit={handleSubmit} className={`space-y-4 ${className}`}>
        <input name="name" type="text" placeholder="Your Name" required value={form.name} onChange={handleChange}
          className={`w-full px-4 py-3 rounded-lg border border-gray-300 focus:outline-none focus:border-${accentColor}-500`} />
        <input name="phone" type="tel" placeholder="Phone Number" required value={form.phone} onChange={handleChange}
          className={`w-full px-4 py-3 rounded-lg border border-gray-300 focus:outline-none focus:border-${accentColor}-500`} />
        <input name="zip" type="text" placeholder="Zip Code" inputMode="numeric" pattern="[0-9]{5}" maxLength={5} required value={form.zip} onChange={handleChange}
          className={`w-full px-4 py-3 rounded-lg border border-gray-300 focus:outline-none focus:border-${accentColor}-500`} />
        <div className="flex items-start gap-3">
          <input type="checkbox" id="lp-optin" name="optin" checked={form.optin} onChange={handleChange}
            className={`mt-1 h-4 w-4 text-${accentColor}-600 border-gray-300 rounded focus:ring-${accentColor}-500`} />
          <label htmlFor="lp-optin" className="text-sm text-gray-600">
            I agree to receive text messages and calls regarding my inquiry. Message and data rates may apply.
          </label>
        </div>
        <button type="submit" disabled={status === 'submitting'}
          className={`w-full bg-${accentColor}-600 hover:bg-${accentColor}-700 text-white font-semibold py-4 rounded-lg transition disabled:opacity-50`}>
          {status === 'submitting' ? 'Submitting...' : 'Get My Free Quote'}
        </button>
  ...
  return (
    <form onSubmit={handleSubmit} className={`space-y-4 ${className}`}>
      ...
      <input name="zip" type="text" placeholder="Zip Code" inputMode="numeric" pattern="[0-9]{5}" maxLength={5} required value={form.zip} onChange={handleChange}
        className={`w-full px-4 py-3 rounded-lg border border-gray-300 focus:outline-none focus:border-${accentColor}-500`} />
      <select name="service" required value={form.service} onChange={handleChange}
        className={`w-full px-4 py-3 rounded-lg border border-gray-300 focus:outline-none focus:border-${accentColor}-500 text-gray-600`}>
        <option value="">Service Needed</option>
        {services.map(s => <option key={s.id} value={s.id}>{s.title}</option>)}
      </select>
      <textarea name="message" placeholder="Tell us about your situation..." rows={4} required value={form.message} onChange={handleChange}
        className={`w-full px-4 py-3 rounded-lg border border-gray-300 focus:outline-none focus:border-${accentColor}-500`} />
      <div className="flex items-start gap-3">
